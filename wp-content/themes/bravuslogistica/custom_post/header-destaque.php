...
// Register Metabox Link Button
add_action("add_meta_boxes", "custom_metabox_link");
function custom_metabox_link(){
    add_meta_box("custom_metabox_link", "Link Button", "custom_metabox_field_link", "header", "normal", "high");
}
...
function custom_metabox_field_link(){
	global $post;
	$data = get_post_custom($post->ID);
	$val = isset($data['custom_input_link']) ? esc_attr($data['custom_input_link'][0]) : 'Não tem Valor!';
    echo '<input type="text" name="custom_input_link" id="custom_input_link" value="'.$val.'" placeholder="Digite aqui!" style="width: 100%" />';
}
...
function salvando(){
	...
	update_post_meta(@$post->ID, "custom_input_link", @$_POST["custom_input_link"]);
}
